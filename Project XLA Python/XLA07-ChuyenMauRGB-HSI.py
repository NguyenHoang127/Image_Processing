...
for x in range(width):
    for y in range(height):
        ...
        #Tính toán các giá trị HSI
        # Tính H - góc theta
        t1 = ((R - G) + (R - B)) / 2
        t2 = math.sqrt(math.pow((R - G), 2) + (R - B) * (G - B))
...
itensity_res = cv.resize(itensity_Cv, dim, interpolation = cv.INTER_AREA)
...
cv.imshow('Kenh Itensity',itensity_res)
cv.imshow('Kenh HSI',hsi_res)
